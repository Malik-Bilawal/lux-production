...
@section('title', 'Add Product Gallery | Admin Panel')
    ...
    <div class="ml-64 flex-1 overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div class="w-full max-w-screen-xl mx-auto">
            <div class="bg-white rounded-2xl shadow p-8">
                ...
                <!-- Form -->
                <form id="productForm" action="{{ route('admin.products.gallery-store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @if($product_id)
...
@endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="url" name="add_to_cart_uri" class="input-field" required placeholder="https://example.com/cart">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Buy Now URL</label>
                            <input type="url" name="buy_now_uri" class="input-field" required placeholder="https://example.com/buy">
                        </div>
                    </div>

                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 width-ful mb-1">Craftsmanship Description</label>
                            <textarea name="craftsmanship_desc" rows="4" class="input-field" required></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Material Description</label>
                            <textarea name="material_desc" rows="4" class="input-field" required></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Key Features</label>
                            <textarea name="key_features" rows="4" class="input-field" required></textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div id="bannerPreview" class="mt-4 max-w-xs h-32 overflow-hidden rounded border border-gray-300"></div>
                        </div>
                        <div>
                            ...
                            <div id="galleryPreview" class="gallery-grid mt-4"></div>
                        </div>
                    </div>
