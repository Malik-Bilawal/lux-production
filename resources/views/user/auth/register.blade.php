...
@section('title', 'Register | Luxorix')
...
<div class="grid-bg h-screen w-full flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <!-- Registration Card -->
        <div class="register-card bg-secondary/80 backdrop-blur-md rounded-2xl border border-accent/20 shadow-2xl shadow-black/30 overflow-hidden w-full max-w-sm compact-mode">
            ...
            <div class="relative z-10">
                <!-- Header -->
                <div class="register-header text-center px-4 pt-6">
                    <div class="register-logo w-14 h-14 mx-auto rounded-full bg-gradient-to-br from-accent3 to-accent flex items-center justify-center mb-2">
                        <i class="fas fa-crown text-xl text-white"></i>
                    </div>
                    <h1 class="text-xl font-bold mb-1">
                        <span class="gradient-text">Create Account</span>
                    </h1>
                    <p class="text-slate-400 text-xs">Join our community and unlock exclusive benefits</p>
                </div>
                
                <!-- Form -->
                <form class="register-form px-4 py-3" method="POST" action="{{ route('register') }}">
                    @csrf
                    
                    <!-- Name -->
                    <div class="form-group mb-3">
                        <label class="form-label block text-slate-300 text-xs font-medium mb-1">Full Name</label>
                        <div class="input-with-icon relative">
                            <i class="fas fa-user input-icon absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-500 text-sm"></i>
                            <input 
                                type="text" 
          ...
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
                    ...
                    <button 
                        ...
                        class="google-signin flex items-center justify-center w-full py-2 rounded-lg bg-primary/50 border border-accent/30 text-slate-300 font-medium text-xs transition-colors duration-300 hover:border-accent"
                    >
                ...
                <!-- Footer -->
                <div class="register-footer px-4 py-3 border-t border-slate-700/50 text-center text-slate-400 text-xs">
                    <p>Already have an account? 
                        <a href="{{ route("login") }}" class="text-accent font-medium hover:text-accent2 transition-colors">Sign In</a>
                    </p>
                </div>
            </div>
        </div>
...
@vite('resources/js/user/auth/register.js')
