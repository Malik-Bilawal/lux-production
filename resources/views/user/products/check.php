...
<body class="bg-theme-page text-theme-content font-body antialiased min-h-screen overflow-x-hidden"
    x-data="productPage()"
    x-init="init()">
...
.scrollbar-hide {
    ...
    scrollbar-width: none; 
        ...
        <!-- Premium Luxury Quantity Selector -->
            <div class="flex hidden items-center justify-between bg-theme-surface border border-theme-deep rounded-xl px-4 py-2 w-full max-w-3xl mx-auto mt-4">
                <!-- Left: Label + Selected Quantity -->
                <div class="flex flex-col">
                    <span class="text-sm font-medium text-theme-muted uppercase tracking-wide">Quantity</span>
                    <span class="text-xs text-theme-muted font-medium mt-0.5">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <button @click="quantity = Math.max(1, quantity - 1)"
                        :disabled="quantity <= 1"
                        class="w-9 h-9 flex items-center justify-center rounded-md bg-theme-deep hover:bg-theme-primary-light disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                        <svg class="w-4 h-4 text-theme-content" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="number"
                        ...
                        @change="quantity = Math.min(Math.max(1, quantity), {{ $product->stock_quantity }})"
                        class="w-14  quantity-input text-center quantity-input bg-theme-deep border border-theme-deep rounded-md text-theme-content font-semibold text-base focus:outline-none focus:ring-2 focus:ring-theme-primary">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <button @click="buyNow({{ $product->id }}, quantity)"
                    ...
                    <span class="text-lg relative z-10" x-text="buyNowLoading ? 'Processing...' : 'Buy Now'"></span>
    ...
    <script>
        ...
        function initSwipers() {
            ...
            if (swiperContainer) {
                ...
                // Expose to window so onclick in thumbnails works
